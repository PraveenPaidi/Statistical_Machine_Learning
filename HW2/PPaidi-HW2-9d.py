...
while not converged:
    #weights train 
    idx = np.random.randint(X_train.shape[0],size=100)
    X_new=X_train[idx, :]
    U_new=1/(1+np.exp(-Y[idx]*(b[idx]+np.dot(X_new,w))))
    wt=w-N*((np.sum((X_new*((U_new-1)*Y[idx])[:, np.newaxis]),axis=0))/100 +2*lam*w)
    epsilon=np.absolute(wt-w)
    w=wt[:]

    #train
    U_new=1/(1+np.exp(-Y[idx]*(b[idx]+np.dot(X_new,w))))
    bt[idx]=b[idx] - N*(U_new-Y[idx])/100    # need to check the condition 
    epsilon1=np.absolute(bt-b)
    b=bt[:]
    
    #test
    idx1 = np.random.randint(X_test.shape[0],size=100)
    X1_new=X_test[idx1, :]
    U1_new=1/(1+np.exp(-Y1[idx1]*(b1[idx1]+np.dot(X1_new,w))))
    bt1[idx1]=b1[idx1] - N*(U1_new-Y1[idx1])/m1
    b1=bt1[:]
